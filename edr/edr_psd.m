...
if options.Plot
    ...
    plot(rv_fit,C*(dr*edrFree).^(2/3).*wv_fit.^slpFree,'-','Color',co(4,:),'LineWidth',2)
    plot(rv_fit,C*(dr*edrFixed).^(2/3).*wv_fit.^slpFixed,'-','Color',co(5,:),'LineWidth',2)

    xlabel('$r\,[\mathrm{m}]$','Interpreter','latex')
    ylabel('$P\,[\mathrm{m^2\,s^{-2}\,rad^{-1}}]$','Interpreter','latex')
    
    legend({'$P$','fit points',...
        ['$p=$ ',num2str(abs(slpFree),'%.2f')],...
        '$p=$ 5/3'},...
