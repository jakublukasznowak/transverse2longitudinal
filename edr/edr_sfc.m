...
if options.Plot
    ...
    plot(rv_fit,C*(rv_fit*edrFree).^slpFree,'-','Color',co(4,:),'LineWidth',1)
    plot(rv_fit,C*(rv_fit*edrFixed).^slpFixed,'-','Color',co(5,:),'LineWidth',1)

    xlabel('$r\,[\mathrm{m}]$','Interpreter','latex')
    ylabel('$D\,[\mathrm{m^2\,s^{-2}}]$','Interpreter','latex')
    
    legend({'structure fun.','fit points',...
